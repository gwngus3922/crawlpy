...
shopname = "비바문"
basicurl = "http://vivamoon.co.kr/" #바꿔
addurl = "product/list.html?cate_no=" #바꿔
pageurl = "&page=" #바꿔
# ,"77","95","25","26","43","27","60","90","28","80"
# ,"BEST","ONLYMOON","OUTER","TOP","BOTTOM","DRESS","SHOES","JEWELRY","ACC","SUMMER"
indexcate = ["24"]
...
def maingo(url,cate,idx):
    ...
    print("".join((basicurl,addurl,idx)))
    nowcate = soup.find("div",{"class" : "tit-product"}).find("h2").find("span").get_text() #바꿔
    if nowcate.upper() != cate:
        print("다르다")
        print(cate)
        print(nowcate)
        global3.errmsg += basicurl+cate+ "변경됨\n"
    menu = soup.find("ul",{"class" : "depth1"}).find_all("li") #바꿔
    ...
    if menu == []:
        ...
        for name in menu:
            try:
                ...
                soncate = name.find("a").find("p").get_text() #바꿔
...
def firstscrape(url,soncate,parentcate):
    ...
    while i < pagenum:
        if i != 0:
            ...
            soup = BeautifulSoup(html,'html.parser')
        try:
            menu = soup.find("ul",{"class" : "prdList grid3"}).find_all("li",{"class" : "item"}) # 추천탭있을때 바꿔
        except IndexError:
            menu = soup.find("ul",{"class" : "prdList grid3"}).find_all("li",{"class" : "item"}) # 없을때 바꿔
        #예외에 대한 처리 (샵마다 다름)
        for name in menu:
            ...
            try:
                ...
                # print(primg)
                prname = name.find("p",{"class" : "name"}).find_all("span")[1].get_text() #이름바꿔
                # print(prname)
                prlink = name.find("div",{"class" : "thumbnail"}).find("a").get("href") #링크 바꿔

                c = name.find("ul",{"class" : "xans-product-listitem"}) #가격 큰 박스부분 바꿔
                prpricebf = c.find("li",{"item-title" :"판매가"}).find_all("span")[1].get_text() #할인전가격 #바꿔
                # print(prpricebf)
                prprice = c.find("li",{"item-title" :"할인판매가"}).find_all("span")[1].get_text() #할인가격 #바꿔
                # print(prprice)
            ...
            try:
                prsoldout = name.find("div",{"class" : "icon"}).find("img",{"alt" : "품절"}) #바꿔 품절
            except AttributeError :
                prsoldout = "0"
            else:
                prsoldout = "1"
...
class detaila():
    ...
    def detailscrapego(url,param,idx,prbf):
        ...
        try:
            ...
            highimage = soup.find("span",{"class" : "detail-image"}).find("img").get("src") #하이이미지 # 바꿔
            # detailimagearray = soup.find("div",{"class":"cont"}).find_all("img") # 디테일 이미지
            detailimagearray = "<head>"
            detailimagearray += '<base href="http://vivamoon.co.kr/" target="_blank">' #바꿔
